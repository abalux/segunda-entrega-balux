...
class ProductManager {
    ...
    async addProduct(title, description, price, thumbnails, code, stock, status, category){
        ...
        if (codeExists){
            ...
            return null
        }else if(!title || !description || !price || !code || !stock || !category || !status){
            console.error("All fields are required")
            return null
        }else{
            const id = this.idGenerator.generateUniqueID();
            const product = new Product(id, title, description, price, thumbnails =  thumbnails || [], code, stock, status = true, category);
    ...
    (async () => {
        ...
        await manager.addProduct("Producto prueba", "Este es un producto prueba", 200, "Sin imagen", "abc123", 25, true, "producto");
        await manager.getProducts();
        /*manager.addProduct("producto prueba", "este es un producto prueba", 200, "sin imagen", "abc123", 25);
        manager.addProduct("producto prueba", "este es un producto prueba", 200, "sin imagen", "abc124");
        await manager.getProductById(1);*/
        manager.addProduct("producto prueba b", "este es un producto prueba b", 300, "sin imagen", "abc125", 25, true, "producto");
        await manager.getProducts();
